...
const SignupScreen = () => {
    ...
    const handleAddEmail = async(email,password)=>{
        try {
            if (email && password) {
                setactiveModal(true)
                ...
                const docRef = doc(collectionRef,user.user.email)
                const create = await setDoc(docRef,{
  ...
  return (
    ...
    <View style={styles.container}>
      ...
      <Formik
        ...
        onSubmit={(values)=>handleAddEmail(values.email,values.password)}
        validateOnMount
      ...
      >
          {({handleBlur,handleChange,handleSubmit,values,errors})=>(
              <View>
                  ...
                  <Text style={{fontSize:15,color:'grey',marginTop:20,alignSelf:'center'}}>Login with existing Email.  <Text onPress={()=>navigation.navigate('LoginScreen')} style={{color:'#4fbff7'}}>Click Here</Text></Text>
              </View>
          )}
      </Formik>
      ...
      >
          <View style={{flex:1,backgroundColor:'black',opacity:.7,justifyContent:'center',alignItems:'center'}}>
            ...
            <Text style={{fontSize:25,fontWeight:'bold',color:'white',marginTop:10,fontFamily:'monospace'}}>Adding...</Text>
          </View>
      </Modal>
